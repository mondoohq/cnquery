...
func newSlackUsers(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "owner":
			if _, ok := val.([]interface{}); !ok {
				return nil, errors.New("Failed to initialize \"slack.users\", its \"owner\" argument has the wrong type (expected type \"[]interface{}\")")
			}
		case "list":
			if _, ok := val.([]interface{}); !ok {
...
func (s *mqlSlackUsers) Owner() ([]interface{}, error) {
	...
	if !ok || !res.Valid {
		...
		res, ok = s.Cache.Load("owner")
		if !ok {
			return nil, errors.New("\"slack.users\" calculated \"owner\" but didn't find its value in cache.")
		}
	...
	tres, ok := res.Data.([]interface{})
	if !ok {
		return nil, fmt.Errorf("\"slack.users\" failed to cast field \"owner\" to the right type ([]interface{}): %#v", res)
	}
	return tres, nil
}
...
func (s *mqlSlackUsers) ComputeOwner() error {
	var err error
