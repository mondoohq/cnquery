...
func newOktaRole(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "description":
			if _, ok := val.(string); !ok {
				return nil, errors.New("Failed to initialize \"okta.role\", its \"description\" argument has the wrong type (expected type \"string\")")
			}
		case "label":
			if _, ok := val.(string); !ok {
...
func (s *mqlOktaRole) Validate() error {
	...
	if _, ok := s.Cache.Load("description"); !ok {
		return errors.New("Initialized \"okta.role\" resource without a \"description\". This field is required.")
	}
...
func (s *mqlOktaRole) Description() (string, error) {
	res, ok := s.Cache.Load("description")
	if !ok || !res.Valid {
		return "", errors.New("\"okta.role\" failed: no value provided for static field \"description\"")
	}
	if res.Error != nil {
		return "", res.Error
	}
	tres, ok := res.Data.(string)
	if !ok {
		return "", fmt.Errorf("\"okta.role\" failed to cast field \"description\" to the right type (string): %#v", res)
	}
	return tres, nil
}
