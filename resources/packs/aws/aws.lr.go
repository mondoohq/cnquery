...
type AwsEc2Instance interface {
	...
	Register(string) error
	Validate() error
	Attachments() (NO_TYPE_DETECTED, error)
	Arn() (string, error)
...
func newAwsEc2Instance(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		switch name {
		case "attachments":
			if _, ok := val.(NO_TYPE_DETECTED); !ok {
				return nil, errors.New("Failed to initialize \"aws.ec2.instance\", its \"attachments\" argument has the wrong type (expected type \"NO_TYPE_DETECTED\")")
			}
		case "arn":
			if _, ok := val.(string); !ok {
...
func (s *mqlAwsEc2Instance) Validate() error {
	// required arguments
	if _, ok := s.Cache.Load("attachments"); !ok {
		return errors.New("Initialized \"aws.ec2.instance\" resource without a \"attachments\". This field is required.")
	}
	if _, ok := s.Cache.Load("arn"); !ok {
...
// Attachments accessor autogenerated
func (s *mqlAwsEc2Instance) Attachments() (NO_TYPE_DETECTED, error) {
	res, ok := s.Cache.Load("attachments")
	if !ok || !res.Valid {
		return nil, errors.New("\"aws.ec2.instance\" failed: no value provided for static field \"attachments\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.(NO_TYPE_DETECTED)
	if !ok {
		return nil, fmt.Errorf("\"aws.ec2.instance\" failed to cast field \"attachments\" to the right type (NO_TYPE_DETECTED): %#v", res)
	}
	return tres, nil
}
