...
func newMacosAlf(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "expections":
			if _, ok := val.([]interface{}); !ok {
				return nil, errors.New("Failed to initialize \"macos.alf\", its \"expections\" argument has the wrong type (expected type \"[]interface{}\")")
			}
		case "explicitAuths":
			if _, ok := val.([]interface{}); !ok {
...
func (s *mqlMacosAlf) Validate() error {
	...
	if _, ok := s.Cache.Load("expections"); !ok {
		return errors.New("Initialized \"macos.alf\" resource without a \"expections\". This field is required.")
	}
...
// Expections accessor autogenerated
func (s *mqlMacosAlf) Expections() ([]interface{}, error) {
	res, ok := s.Cache.Load("expections")
	if !ok || !res.Valid {
		return nil, errors.New("\"macos.alf\" failed: no value provided for static field \"expections\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.([]interface{})
	if !ok {
		return nil, fmt.Errorf("\"macos.alf\" failed to cast field \"expections\" to the right type ([]interface{}): %#v", res)
	}
	return tres, nil
}
