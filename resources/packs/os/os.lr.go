...
func newRegistrykey(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "props":
			if _, ok := val.(map[string]interface{}); !ok {
				return nil, errors.New("Failed to initialize \"registrykey\", its \"props\" argument has the wrong type (expected type \"map[string]interface{}\")")
			}
		case "items":
			if _, ok := val.([]interface{}); !ok {
...
// Props accessor autogenerated
func (s *mqlRegistrykey) Props() (map[string]interface{}, error) {
	res, ok := s.Cache.Load("props")
	if !ok || !res.Valid {
		if err := s.ComputeProps(); err != nil {
			return nil, err
		}
		res, ok = s.Cache.Load("props")
		if !ok {
			return nil, errors.New("\"registrykey\" calculated \"props\" but didn't find its value in cache.")
		}
		s.MotorRuntime.Trigger(s, "props")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.(map[string]interface{})
	if !ok {
		return nil, fmt.Errorf("\"registrykey\" failed to cast field \"props\" to the right type (map[string]interface{}): %#v", res)
	}
	return tres, nil
}
...
// ComputeProps computer autogenerated
func (s *mqlRegistrykey) ComputeProps() error {
	var err error
	if _, ok := s.Cache.Load("props"); ok {
		return nil
	}
	vres, err := s.GetProps()
	if _, ok := err.(resources.NotReadyError); ok {
	...
	s.Cache.Store("props", &resources.CacheEntry{Data: vres, Valid: true, Error: err, Timestamp: time.Now().Unix()})
	return nil
}
