...
func newOpcuaNode(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "value":
			if _, ok := val.(interface{}); !ok {
				return nil, errors.New("Failed to initialize \"opcua.node\", its \"value\" argument has the wrong type (expected type \"interface{}\")")
			}
		case "__id":
			idVal, ok := val.(string)
			if !ok {
...
func (s *mqlOpcuaNode) Value() (interface{}, error) {
	...
	if !ok || !res.Valid {
		...
		res, ok = s.Cache.Load("value")
		if !ok {
			return nil, errors.New("\"opcua.node\" calculated \"value\" but didn't find its value in cache.")
		}
	...
	tres, ok := res.Data.(interface{})
	if !ok {
		return nil, fmt.Errorf("\"opcua.node\" failed to cast field \"value\" to the right type (interface{}): %#v", res)
	}
	return tres, nil
}
...
func (s *mqlOpcuaNode) ComputeValue() error {
	var err error
