...
func (g *mqlGcpProjectIamServiceServiceAccount) GetRoles() ([]interface{}, error) {
	projectId, err := g.ProjectId()
	if err != nil {
		return nil, err
	}
	...
	mqlRoles := make([]interface{}, 0, len(resp.Keys))
	for _, k := range resp.Keys {
		mqlRole, err := g.MotorRuntime.CreateResource("gcp.project.iamService.serviceAccount.role",
			"name", k.Name,
			"keyAlgorithm", k.KeyAlgorithm.String(),
			"validAfterTime", timestampAsTimePtr(k.ValidAfterTime),
			"validBeforeTime", timestampAsTimePtr(k.ValidBeforeTime),
