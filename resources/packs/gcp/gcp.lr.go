...
func newGcpEssentialContact(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "name":
			if _, ok := val.(string); !ok {
				return nil, errors.New("Failed to initialize \"gcp.essentialContact\", its \"name\" argument has the wrong type (expected type \"string\")")
			}
...
func (s *mqlGcpEssentialContact) Validate() error {
	...
	if _, ok := s.Cache.Load("name"); !ok {
		return errors.New("Initialized \"gcp.essentialContact\" resource without a \"name\". This field is required.")
	}
...
// Name accessor autogenerated
func (s *mqlGcpEssentialContact) Name() (string, error) {
	res, ok := s.Cache.Load("name")
	if !ok || !res.Valid {
		return "", errors.New("\"gcp.essentialContact\" failed: no value provided for static field \"name\"")
	}
	if res.Error != nil {
		return "", res.Error
	}
	tres, ok := res.Data.(string)
	if !ok {
		return "", fmt.Errorf("\"gcp.essentialContact\" failed to cast field \"name\" to the right type (string): %#v", res)
	}
	return tres, nil
}
