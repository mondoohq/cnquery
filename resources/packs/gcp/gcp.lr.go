...
type GcpProjectApiKeyRestrictionServerKeyRestrictions interface {
	...
	Validate() error
	Id() (string, error)
	AllowedAddresses() ([]interface{}, error)
}
...
func newGcpProjectApiKeyRestrictionServerKeyRestrictions(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "allowedAddresses":
			if _, ok := val.([]interface{}); !ok {
				return nil, errors.New("Failed to initialize \"gcp.project.apiKey.restriction.serverKeyRestrictions\", its \"allowedAddresses\" argument has the wrong type (expected type \"[]interface{}\")")
			}
		case "__id":
			idVal, ok := val.(string)
			if !ok {
...
func (s *mqlGcpProjectApiKeyRestrictionServerKeyRestrictions) Validate() error {
	...
	if _, ok := s.Cache.Load("allowedAddresses"); !ok {
		return errors.New("Initialized \"gcp.project.apiKey.restriction.serverKeyRestrictions\" resource without a \"allowedAddresses\". This field is required.")
	}
...
// AllowedAddresses accessor autogenerated
func (s *mqlGcpProjectApiKeyRestrictionServerKeyRestrictions) AllowedAddresses() ([]interface{}, error) {
	res, ok := s.Cache.Load("allowedAddresses")
	if !ok || !res.Valid {
		return nil, errors.New("\"gcp.project.apiKey.restriction.serverKeyRestrictions\" failed: no value provided for static field \"allowedAddresses\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.([]interface{})
	if !ok {
		return nil, fmt.Errorf("\"gcp.project.apiKey.restriction.serverKeyRestrictions\" failed to cast field \"allowedAddresses\" to the right type ([]interface{}): %#v", res)
	}
	return tres, nil
}
