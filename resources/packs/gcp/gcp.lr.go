...
func newGcpComputeInstance(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "confidentialInstanceConfig":
			if _, ok := val.(interface{}); !ok {
				return nil, errors.New("Failed to initialize \"gcp.compute.instance\", its \"confidentialInstanceConfig\" argument has the wrong type (expected type \"interface{}\")")
			}
		case "canIpForward":
			if _, ok := val.(bool); !ok {
...
func (s *mqlGcpComputeInstance) Validate() error {
	...
	if _, ok := s.Cache.Load("confidentialInstanceConfig"); !ok {
		return errors.New("Initialized \"gcp.compute.instance\" resource without a \"confidentialInstanceConfig\". This field is required.")
	}
...
func (s *mqlGcpComputeInstance) ConfidentialInstanceConfig() (interface{}, error) {
	res, ok := s.Cache.Load("confidentialInstanceConfig")
	if !ok || !res.Valid {
		return nil, errors.New("\"gcp.compute.instance\" failed: no value provided for static field \"confidentialInstanceConfig\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.(interface{})
	if !ok {
		return nil, fmt.Errorf("\"gcp.compute.instance\" failed to cast field \"confidentialInstanceConfig\" to the right type (interface{}): %#v", res)
	}
	return tres, nil
}
