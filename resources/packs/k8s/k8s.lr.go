...
func newK8sKubelet(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "options":
			if _, ok := val.(interface{}); !ok {
				return nil, errors.New("Failed to initialize \"k8s.kubelet\", its \"options\" argument has the wrong type (expected type \"interface{}\")")
			}
		case "__id":
			idVal, ok := val.(string)
			if !ok {
...
func (s *mqlK8sKubelet) Validate() error {
	...
	if _, ok := s.Cache.Load("options"); !ok {
		return errors.New("Initialized \"k8s.kubelet\" resource without a \"options\". This field is required.")
	}
...
func (s *mqlK8sKubelet) Options() (interface{}, error) {
	res, ok := s.Cache.Load("options")
	if !ok || !res.Valid {
		return nil, errors.New("\"k8s.kubelet\" failed: no value provided for static field \"options\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.(interface{})
	if !ok {
		return nil, fmt.Errorf("\"k8s.kubelet\" failed to cast field \"options\" to the right type (interface{}): %#v", res)
	}
	return tres, nil
}
