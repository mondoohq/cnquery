...
func newK8sIngress(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	...
	for name, val := range *args {
		...
		case "spec":
			if _, ok := val.(interface{}); !ok {
				return nil, errors.New("Failed to initialize \"k8s.ingress\", its \"spec\" argument has the wrong type (expected type \"interface{}\")")
			}
		case "__id":
			idVal, ok := val.(string)
			if !ok {
...
func (s *mqlK8sIngress) Validate() error {
	...
	if _, ok := s.Cache.Load("spec"); !ok {
		return errors.New("Initialized \"k8s.ingress\" resource without a \"spec\". This field is required.")
	}
...
// Spec accessor autogenerated
func (s *mqlK8sIngress) Spec() (interface{}, error) {
	res, ok := s.Cache.Load("spec")
	if !ok || !res.Valid {
		return nil, errors.New("\"k8s.ingress\" failed: no value provided for static field \"spec\"")
	}
	if res.Error != nil {
		return nil, res.Error
	}
	tres, ok := res.Data.(interface{})
	if !ok {
		return nil, fmt.Errorf("\"k8s.ingress\" failed to cast field \"spec\" to the right type (interface{}): %#v", res)
	}
	return tres, nil
}
