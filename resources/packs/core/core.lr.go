...
func Init(registry *resources.Registry) {
	...
	registry.AddFactory("pythonPackages", newPythonPackages)
}
...
// PythonPackages resource interface
type PythonPackages interface {
	MqlResource() (*resources.Resource)
	MqlCompute(string) error
	Field(string) (interface{}, error)
	Register(string) error
...
// mqlPythonPackages for the pythonPackages resource
type mqlPythonPackages struct {
	*resources.Resource
}

// MqlResource to retrieve the underlying resource info
func (s *mqlPythonPackages) MqlResource() *resources.Resource {
	return s.Resource
}

// create a new instance of the pythonPackages resource
func newPythonPackages(runtime *resources.Runtime, args *resources.Args) (interface{}, error) {
	// User hooks
	var err error
	res := mqlPythonPackages{runtime.NewResource("pythonPackages")}
	// assign all named fields
	var id string
	...
	for name, val := range *args {
		...
		case "list":
			if _, ok := val.([]interface{}); !ok {
				return nil, errors.New("Failed to initialize \"pythonPackages\", its \"list\" argument has the wrong type (expected type \"[]interface{}\")")
			}
		case "__id":
			idVal, ok := val.(string)
			if !ok {
				return nil, errors.New("Failed to initialize \"pythonPackages\", its \"__id\" argument has the wrong type (expected type \"string\")")
			}
			id = idVal
		default:
			return nil, errors.New("Initialized pythonPackages with unknown argument " + name)
		}
		res.Cache.Store(name, &resources.CacheEntry{Data: val, Valid: true, Timestamp: now})
	}
...
func (s *mqlPythonPackages) Validate() error {
	// required arguments
	// no required fields found
...
// Register accessor autogenerated
func (s *mqlPythonPackages) Register(name string) error {
	log.Trace().Str("field", name).Msg("[pythonPackages].Register")
	switch name {
	case "list":
		return nil
	default:
		return errors.New("Cannot find field '" + name + "' in \"pythonPackages\" resource")
	}
}

// Field accessor autogenerated
func (s *mqlPythonPackages) Field(name string) (interface{}, error) {
	log.Trace().Str("field", name).Msg("[pythonPackages].Field")
	switch name {
	case "list":
		return s.List()
	default:
		return nil, fmt.Errorf("Cannot find field '" + name + "' in \"pythonPackages\" resource")
	}
}

// List accessor autogenerated
func (s *mqlPythonPackages) List() ([]interface{}, error) {
	res, ok := s.Cache.Load("list")
	if !ok || !res.Valid {
		if err := s.ComputeList(); err != nil {
			return nil, err
		...
		res, ok = s.Cache.Load("list")
		if !ok {
			return nil, errors.New("\"pythonPackages\" calculated \"list\" but didn't find its value in cache.")
		}
		s.MotorRuntime.Trigger(s, "list")
	}
	if res.Error != nil {
	...
	tres, ok := res.Data.([]interface{})
	if !ok {
		return nil, fmt.Errorf("\"pythonPackages\" failed to cast field \"list\" to the right type ([]interface{}): %#v", res)
	}
	return tres, nil
}

// Compute accessor autogenerated
func (s *mqlPythonPackages) MqlCompute(name string) error {
	log.Trace().Str("field", name).Msg("[pythonPackages].MqlCompute")
	switch name {
	case "list":
		return s.ComputeList()
	default:
...
// ComputeList computer autogenerated
func (s *mqlPythonPackages) ComputeList() error {
	var err error
	if _, ok := s.Cache.Load("list"); ok {
		return nil
	}
