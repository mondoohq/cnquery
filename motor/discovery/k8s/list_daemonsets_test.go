...
func TestListDaemonsets(t *testing.T) {
	...
	expectedAssetPlatformIds := []string{
		clusterIdentifier + "/namespace/" + nss[0].Name + "/daemonsets/name/" + daemonsets[0].Name + "/uid/" + string(daemonsets[0].UID),
		clusterIdentifier + "/namespace/" + nss[0].Name + "/daemonsets/name/" + daemonsets[1].Name + "/uid/" + string(daemonsets[1].UID),
	}
