...
func TestListPodImage(t *testing.T) {
	...
	transport := fake.NewFakeTransport(mockCtrl)
	...
	transport.MockConnector.EXPECT().Namespaces().Return(nss, nil)
	...
	transport.MockConnector.EXPECT().Pods(nss[0]).Return(pods1, nil)
	transport.MockConnector.EXPECT().Pods(nss[1]).Return(pods2, nil)
