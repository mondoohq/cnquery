...
func ListAssets(runtimes ...string) ([]*assets.Asset, error) {
	...
	if stringslice.Contains(runtimes, runtime.RUNTIME_AWS_EC2) ||
		stringslice.Contains(runtimes, runtime.RUNTIME_AWS_SSM_MANAGED) ||
		stringslice.Contains(runtimes, runtime.RUNTIME_AWS_ECR) {
		...
		if err != nil {
			...
			if stringslice.Contains(runtimes, runtime.RUNTIME_AWS_SSM_MANAGED) {
			...
			if stringslice.Contains(runtimes, runtime.RUNTIME_AWS_ECR) {
	...
	if stringslice.Contains(runtimes, runtime.RUNTIME_DOCKER_CONTAINER) {
		askRuntimes = append(askRuntimes, &docker.Container{})
	}

	if stringslice.Contains(runtimes, runtime.RUNTIME_DOCKER_IMAGE) {
