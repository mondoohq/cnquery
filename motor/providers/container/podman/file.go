...
func FileOpen(podmanClient context.Context, path string, container string, transport *Provider) (afero.File, error) {
	f := &File{
		path:         path,
...
func (f *File) getFileDockerReader(path string) (io.ReadCloser, dockertypes.ContainerPathStat, error) {
	r, stat, err := containers.CopyToArchive(f.podmanClient,f.container,path,
