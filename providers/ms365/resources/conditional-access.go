...
func (a *mqlMicrosoftConditionalAccessPolicies) createConditionsResource(
	...
	if conditions != nil && conditions.GetApplications() != nil {
		...
		mqlApplications, err = CreateResource(a.MqlRuntime, "microsoft.conditionalAccess.policy.conditions.applications", map[string]*llx.RawData{
			...
			"includeAuthenticationContextClassReferences": llx.ArrayData(convert.SliceAnyToInterface(apps.GetIncludeAuthenticationContextClassReferences()), types.String),
		})
		if err != nil {
			return nil, err
		}
