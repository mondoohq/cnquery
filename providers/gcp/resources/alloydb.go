...
func (g *mqlGcpProjectAlloydbService) clusters() ([]any, error) {
	...
	for {
		...
		mqlCluster, err := CreateResource(g.MqlRuntime, "gcp.project.alloydbService.cluster", map[string]*llx.RawData{
			...
			"deleteProtectionEnabled": llx.BoolData(false),
...
func parseAlloyDBClusterName(name string) *alloyDBClusterParts {
	...
	parts := parseResourceNameParts(name, 6)
	if parts == nil {
		return nil
	}
...
func parseResourceNameParts(name string, expectedParts int) []string {
	parts := make([]string, 0)
	for _, p := range splitResourceName(name) {
