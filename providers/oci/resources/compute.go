		...
		f := func() (jobpool.JobResult, error) {
			...
			for i := range volumes {
				...
				mqlInstance, err := CreateResource(o.MqlRuntime, "oci.compute.blockVolume", map[string]*llx.RawData{
					...
					"sizeInGBs":          llx.IntData(int64Value(vol.SizeInGBs)),
					"vpusPerGB":          llx.IntData(int64Value(vol.VpusPerGB)),
					"state":              llx.StringData(string(vol.LifecycleState)),
					"isHydrated":         llx.BoolData(boolValue(vol.IsHydrated)),
					"kmsKeyId":           llx.StringDataPtr(vol.KmsKeyId),
					"isAutoTuneEnabled":  llx.BoolData(boolValue(vol.IsAutoTuneEnabled)),
					"created":            llx.TimeDataPtr(created),
				})
				if err != nil {
					return nil, err
		...
		f := func() (jobpool.JobResult, error) {
			...
			for i := range bootVols {
				...
				mqlInstance, err := CreateResource(o.MqlRuntime, "oci.compute.bootVolume", map[string]*llx.RawData{
					...
					"sizeInGBs":          llx.IntData(int64Value(bv.SizeInGBs)),
