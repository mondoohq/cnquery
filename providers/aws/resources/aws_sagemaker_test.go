...
func TestSagemakerTagsCacheDoubleCheckLocking(t *testing.T) {
	...
	// After pre-fetching, the cache should not be nil
...
func TestBuildProductionVariantsWithData(t *testing.T) {
	...
	// We can't test with a real runtime, but we can verify the function doesn't panic on nil runtime
	// when given empty variants
	emptyResult, err := sagemakerBuildProductionVariants(nil, "parent", nil)
	require.NoError(t, err)
	assert.Empty(t, emptyResult)

	// Verify variant data would be processed (can't fully test without runtime)
...
func TestNotebookInstanceDetailsNullSubnet(t *testing.T) {
	...
	// Verify the struct can be created with nil cacheSubnetId
