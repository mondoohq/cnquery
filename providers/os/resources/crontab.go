...
import (
	...
	"go.mondoo.com/cnquery/v12/providers/os/resources/crontab"
)
...
func (c *mqlCrontab) entries() ([]any, error) {
	...
	for _, path := range systemCrontabPaths {
		entries, fileRes, err := c.parseCrontabFile(fs, path, true, "")
		if err != nil {
	...
	for _, dir := range systemCronDirs {
		entries, files, err := c.parseCronDir(fs, dir, true)
		if err != nil {
			continue
		}
		allEntries = append(allEntries, entries...)
	...
	for _, dir := range userCrontabDirs {
		entries, files, err := c.parseUserCrontabDir(fs, dir)
		if err != nil {
			continue
		}
		allEntries = append(allEntries, entries...)
...
func (c *mqlCrontab) files() ([]any, error) {
	...
	_, err := c.GetEntries().Data, c.GetEntries().Error
	if err != nil {
		return nil, err
	}
...
func (c *mqlCrontab) parseCrontabFile(fs shared.Fs, path string, hasUserField bool, defaultUser string) ([]any, plugin.Resource, error) {
	f, err := fs.Open(path)
	if err != nil {
		return nil, nil, err
	}
...
func (c *mqlCrontab) parseCronDir(fs shared.Fs, dir string, hasUserField bool) ([]any, []any, error) {
	...
	for _, file := range files {
		...
		entries, fileRes, err := c.parseCrontabFile(fs, path, hasUserField, "")
		if err != nil {
			continue
		}
		allEntries = append(allEntries, entries...)
...
func (c *mqlCrontab) parseUserCrontabDir(fs shared.Fs, dir string) ([]any, []any, error) {
	files, err := fs.ReadDir(dir)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, file := range files {
		...
		entries, fileRes, err := c.parseCrontabFile(fs, path, false, username)
		if err != nil {
			continue
		}
		allEntries = append(allEntries, entries...)
