// Copyright (c) Mondoo, Inc.
// SPDX-License-Identifier: BUSL-1.1

package plist

import (
	"bytes"
	"encoding/base64"
	"os"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestDecodePlist(t *testing.T) {
	data := ""

	decodedData, err := base64.StdEncoding.DecodeString(data)
	require.NoError(t, err)

	t.Run("roundtrip conversion", func(t *testing.T) {
		plistData, err := Decode(bytes.NewReader(decodedData))
		require.NoError(t, err)
		assert.NotNil(t, plistData)
		assert.Equal(t, "login text", plistData["LoginwindowText"])

		// convert binary to xml and parse again
		plistXmlData, err := ToXml(bytes.NewReader(decodedData))
		require.NoError(t, err)
		assert.NotNil(t, plistXmlData)

		plistData, err = Decode(bytes.NewReader(decodedData))
		require.NoError(t, err)
		assert.NotNil(t, plistData)
		assert.Equal(t, "login text", plistData["LoginwindowText"])
	})

	t.Run("test get value", func(t *testing.T) {
		plistData, err := Decode(bytes.NewReader(decodedData))
		require.NoError(t, err)

		str, ok := plistData.GetString("LoginwindowText")
		assert.True(t, ok, "expected to pass")
		assert.Equal(t, "login text", str)

		// is a number, so this returns false
		str, ok = plistData.GetString("AccountInfo", "MaximumUsers")
		assert.False(t, ok, "expected to fail")
		assert.Equal(t, "", str)

		number, ok := plistData.GetNumber("AccountInfo", "MaximumUsers")
		assert.True(t, ok, "expected to pass")
		assert.Equal(t, float64(1), number)
	})

	t.Run("test List", func(t *testing.T) {
		decodedData, err := os.ReadFile("testdata/list.xml")
		require.NoError(t, err)

		plistData, err := Decode(bytes.NewReader(decodedData))
		require.NoError(t, err)

		items, ok := plistData.GetList("_computerlevel")
		assert.True(t, ok)
		assert.Equal(t, 1, len(items))
	})

}
