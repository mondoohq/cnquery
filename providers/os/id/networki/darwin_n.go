		...
		if currentInterface != nil {
			...
			if strings.HasPrefix(line, "inet6 ") {
				...
				if len(fields) > 1 {
					...
					if len(fields) > 3 {
						...
						ipv6 = NewIPv6WithPrefixLenght(ip, prefixLength)
					}
			...
			if strings.Contains(line, "status:") {
				...
				if len(fields) > 1 {
					if fields[1] == "active" {
						active := true
						currentInterface.Active = &active
