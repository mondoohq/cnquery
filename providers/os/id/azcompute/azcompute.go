...
const (
	instanceMetadataURL           = "http://169.254.169.254/metadata/instance?api-version=2021-02-01"
	...
	loadbalancerMetadataURL           = "http://169.254.169.254/metadata/loadbalancer?api-version=2021-02-01"
...
func (m *commandInstanceMetadata) instanceDocument() ([]byte, error) {
	...
	case m.platform.IsFamily(inventory.FAMILY_UNIX):
		cmd, err = m.conn.RunCommand("curl --retry 5 --retry-delay 1 --connect-timeout 1 --retry-max-time 5 --max-time 10 --noproxy '*' -H Metadata:true " + instanceMetadataURL)
...
func (m *commandInstanceMetadata) loadbalancerDocument() ([]byte, error) {
	...
	switch {
	case m.platform.IsFamily(inventory.FAMILY_UNIX):
		cmd, err = m.conn.RunCommand("curl --retry 5 --retry-delay 1 --connect-timeout 1 --retry-max-time 5 --max-time 10 --noproxy '*' -H Metadata:true " + loadbalancerMetadataURL)
	case m.platform.IsFamily(inventory.FAMILY_WINDOWS):
