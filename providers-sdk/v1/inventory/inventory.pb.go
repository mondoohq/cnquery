...
type Asset struct {
	...
	// name_override marks `name` as a name the caller explicitly asked for --
	// cnspec's --asset-name, or a `name:` on an asset in an inventory file --
	// rather than a default a provider filled in during ParseCLI. Discovery puts
	// such a name back on the asset after connecting, because a provider's detect
	// step overwrites it. Without this marker the two are the same string in the
	// same field, and a provider default outranks the name detect computed.
	NameOverride  bool `protobuf:"varint,40,opt,name=name_override,json=nameOverride,proto3" json:"name_override,omitempty"`
...
func (x *Asset) GetNameOverride() bool {
	if x != nil {
...
const file_inventory_proto_rawDesc = "" +
	"\n" +
	"\x0finventory.proto\x12\x14cnquery.providers.v1\x1a(providers-sdk/v1/upstream/upstream.proto\x1a\"providers-sdk/v1/vault/vault.proto\"\xbc\b\n" +
	"\x05Asset\x12\x0e\n" +
	...
	"\rrelationships\x18' \x03(\v2'.cnquery.providers.v1.AssetRelationshipR\rrelationships\x12#\n" +
	"\rname_override\x18( \x01(\bR\fnameOverride\x1a9\n" +
	...
	"\x17technology_url_segments\x18\t \x03(\tR\x15technologyUrlSegments\x12\x18\n" +
