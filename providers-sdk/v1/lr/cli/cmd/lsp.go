...
func runLSPServer(debug bool) error {
	...
	if debug {
		logrus.Debug("Starting LSP server with debug mode")
	}

	logrus.Info("LR LSP server starting - reading from stdin, writing to stdout")
...
func (h *LRHandler) textDocumentDefinition(context *glsp.Context, params *protocol.DefinitionParams) (any, error) {
	...
	logrus.WithField("locations", len(locations)).Debug("Found definition locations")
...
func (h *LRHandler) findDefinitionLocations(doc *Document, word string) []protocol.Location {
	...
	logrus.WithField("word", word).Debug("Finding definition for word")

	// Search for resource definitions
	for _, resource := range doc.AST.Resources {
		if resource.ID == word {
			logrus.WithField("resource", resource.ID).Debug("Found matching resource in AST")
					...
					if isDefinition {
						...
						if start != -1 {
							...
							logrus.WithFields(logrus.Fields{
								"line":    i,
								"start":   start,
								"pattern": "resource definition with attributes",
							}).Debug("Found resource definition")
							goto nextResource
						}
					}
				}
	...
	for _, resource := range doc.AST.Resources {
		if resource.Body != nil {
			for _, field := range resource.Body.Fields {
				if field.BasicField != nil && field.BasicField.ID == word {
					logrus.WithFields(logrus.Fields{
						"field":    field.BasicField.ID,
						"resource": resource.ID,
					}).Debug("Found matching field in AST")
