...
func TestRedhat7Parser(t *testing.T) {
	mock, err := mock.NewFromToml(&transports.Endpoint{Backend: "mock", Path: "./testdata/packages_redhat7.toml"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRedhat6Parser(t *testing.T) {
	mock, err := mock.NewFromToml(&transports.Endpoint{Backend: "mock", Path: "./testdata/packages_redhat6.toml"})
	if err != nil {
		t.Fatal(err)
	}
