...
func TestAlpineApkdbParser(t *testing.T) {
	mock, err := mock.NewFromToml(&transports.Endpoint{Backend: "mock", Path: "./testdata/packages_apk.toml"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApkUpdateParser(t *testing.T) {
	mock, err := mock.NewFromToml(&transports.Endpoint{Backend: "mock", Path: "./testdata/updates_apk.toml"})
	if err != nil {
		t.Fatal(err)
	}
