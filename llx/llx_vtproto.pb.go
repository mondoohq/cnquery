...
func (m *CodeBundle) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.MinProviderVersions) > 0 {
		for k := range m.MinProviderVersions {
			v := m.MinProviderVersions[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
	...
	if len(m.ProviderSchemas) > 0 {
		for k := range m.ProviderSchemas {
			v := m.ProviderSchemas[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(v)))
...
func (m *TranslationRef) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *TranslationRef) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TranslationRef) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.BlockRef != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.BlockRef))
		i--
		dAtA[i] = 0x20
	}
	if len(m.BelowVersion) > 0 {
		i -= len(m.BelowVersion)
		copy(dAtA[i:], m.BelowVersion)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.BelowVersion)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Provider) > 0 {
...
func (m *CodeBundle) SizeVT() (n int) {
	...
	if len(m.ProviderSchemas) > 0 {
		for k, v := range m.ProviderSchemas {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + len(v) + protohelpers.SizeOfVarint(uint64(len(v)))
			n += mapEntrySize + 2 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
	...
	if len(m.MinProviderVersions) > 0 {
		for k, v := range m.MinProviderVersions {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + len(v) + protohelpers.SizeOfVarint(uint64(len(v)))
			n += mapEntrySize + 2 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
...
func (m *TranslationRef) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	l = len(m.BelowVersion)
	if l > 0 {
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
	}
	if m.BlockRef != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.BlockRef))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *CodeBundle) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 26:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ProviderSchemas", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.ProviderSchemas == nil {
				m.ProviderSchemas = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.ProviderSchemas[mapkey] = mapvalue
			iNdEx = postIndex
		case 27:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinProviderVersions", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.MinProviderVersions == nil {
				m.MinProviderVersions = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.MinProviderVersions[mapkey] = mapvalue
			iNdEx = postIndex
		case 28:
			if wireType != 2 {
			...
			m.Translations = append(m.Translations, &TranslationRef{})
...
func (m *TranslationRef) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TranslationRef: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TranslationRef: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field BelowVersion", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.BelowVersion = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 4:
			if wireType != 0 {
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.BlockRef |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
