...
4. **Cross-asset resolution in `providers.Runtime`.** When resolving a field chain
   ...
   **This path is also the one imperative Go-side callers use.** A provider that
   ...
   separate asset — "Mode 2" in
   [ADR 042](042-cross-provider-invocation.md#the-three-shapes-of-cross-calling))
   goes through this same resolution, exposed as a thin SDK-side wrapper rather
   than a parallel API. A second connect path would bypass the recording-first
   ordering above and make those assets invisible to replay, which is precisely
   what this ADR exists to avoid. Note this is *not* the same-asset cross-provider
   call gated by ADR 042 — no `Offers` declaration is involved, because creating a
   genuinely new asset is not "being co-opted onto someone else's asset."
5. **Namespace evolution (v14 → v15).** This ADR is part of the v14 line.
   ...
   **Cross-provider call legality is not decided here.** Whether an undeclared
   ...
   two declaration forms — typed field here, manifest `Offers` there — share one
   enforcement date.
...
3. **Live-connect backend.** Add the `RuntimeFor`+`Connect`+resolve+close path for
   ...
   wrapper (ADR 042's "Mode 2") over this same path once it lands — additive, and
   registering the produced asset as a discovered child reuses the existing
   discovery plumbing.
