# ADR 033: Late binding of resource types (stable type identity)
...
[ADR 032](032-cross-version-type-migration.md) reconciles version-skewed content
...
Late binding is **additive to ADR 032, not a replacement.** ADR 032 handles
...
name. Fix that and the vast majority of ADR 032's lenses become unnecessary: they
collapse from "authored transforms" to "nothing to do."

This ADR **depends on ADR 032 shipping first.** Introducing stable identity is a
wire change, and the only safe way to roll it out across a skewed fleet is behind
ADR 032's min-MQL-version axis and down-adaptation. So 032 is the foundation;
033 is the follow-on that shrinks 032's ongoing cost.
...
2. **Content references ids, not names.** Compiled bytecode carries the stable id
   ...
   for diagnostics and decompilation. ADR 032's migration-chain-canonical bundle
...
4. **Roll out behind ADR 032's version axis.** New (v14+) engines emit and consume
   id-based content; older engines receive name-based content via ADR 032
   ...
   coordination. ADR 032's min-MQL-version axis is exactly what gates this.
...
5. **Cross-provider unification stays explicit, not inferred.** Merging
   ...
   032 lens/alias, shape-checked by the part-5 build gate), **not** something the
...
2. **Dual-encode content** (id + name) in the compiler; make the runtime resolve
   by id with name fallback. Redefine ADR 032's canonical identity as the id set.
...
4. **Retire name-based resolution** once the fleet's min-MQL-version floor clears
   the transition window (ADR 032 governs the floor).

## Consequences

- Renames, field moves, and reorganizations become **non-breaking by
  construction**, eliminating the largest and most avoidable class of ADR 032
...
- It is a wire/format change touching every serialized bundle, DB blob, and
  `resources.json`. Bounded and safe only because ADR 032's provenance +
  down-adaptation carry the transition — which is why 032 lands first.
...
- **Structural / shape typing** (identity = the set of field name→types, not a
  ...
  *changes* still break (so ADR 032 is still needed); and it is a far larger VM
...
- **Keep baking, rely on ADR 032 lenses alone** (do nothing here). Every rename
  and reorg remains a hand-written lens forever, and bundle identity keeps
  fragmenting on cosmetic changes. This is the status quo ADR 032 leaves behind;
...
- **Split / merge of resources** (one resource becoming two, or the `git.repo`
  ...
  boundary where stable ids hand off to ADR 032's lenses.
